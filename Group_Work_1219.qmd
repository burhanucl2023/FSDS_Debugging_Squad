...
**Conclusion: We can set a special number to filter all rows which price is larger than this number**

## Convert the format of 'price'
...
## Filter too large number in 'price'
...
print(rows_gt_2000['price'])
print(f"The number of row which 'price' is larger than '2000'：{count_gt_2000} rows")
```

```{python}
shit = rows_gt_2000[rows_gt_2000['number_of_reviews_ltm']>0]

print(shit[['price', 'number_of_reviews_ltm', 'property_type', 'room_type']])

#print(rows_gt_2000[['price', 'number_of_reviews_ltm', 'property_type', 'room_type']])
```
...
price_histogram = listings_df1[listings_df1['price'] < 2000]['price'].plot.hist(bins=200)
plt.xlabel('Price')
...
# selected_data 现在包含了1%到99%分位数范围内的数据
```
...
# 创建箱线图（竖向）
plt.boxplot(data, vert=True)  # vert=True 表示竖向箱线图

# 创建数据点的x坐标（沿着纵坐标排列）
#x_coordinates = np.ones(len(data))
x_coordinates = np.random.normal(1, 0.04, size=len(data))  # 添加轻微的扰动以避免重叠
...
# 标注最小值、第一四分位数、中位数、第三四分位数和最大值的数值
...
# Host type_Host中local host占总数的多少
...
# Since we cannot directly access the content of the image, let's assume that the dataframe is available as `df`.
# The actual implementation would require converting the image data to a pandas DataFrame.
# The following is a placeholder for the actual DataFrame creation process.
...
plt.barh(merged_sorted_df['neighbourhood_cleansed'], merged_sorted_df['contribution_per_listing'], color = colors)
plt.xlabel('contribution_per_listing', fontsize = 12)
plt.ylabel('Neighbourhood', fontsize = 12)
plt.title('Economic Contribution in London Boroughs (per listing)', fontsize = 20)

# Create a color string
sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
sm.set_array([])  # Just show the color string
plt.colorbar(sm)
...
```{python}
merged_sorted1_df = merged_df.sort_values('contribution_per_listing', ascending=False)
# Plotting the vertical bar chart
plt.figure(figsize=(12, 8))
plt.bar(merged_sorted1_df['neighbourhood_cleansed'], merged_sorted1_df['contribution_per_listing'], color='skyblue')
plt.ylabel('Ratio')
plt.xlabel('Neighbourhood')
plt.title('Ratio by Neighbourhood')
plt.xticks(rotation=90)  # Rotate x-axis labels to ensure they're readable
plt.tight_layout()
plt.show()
```
...
merged_occupancy_gdf.plot(column='avarage_occupancy_day', ax=ax, legend=True, cmap='RdBu_r', edgecolor='white',
                legend_kwds={'label': "Ratio by Borough",
                             'orientation': "vertical", 
                             'shrink': 0.7})
# Show the plot
plt.show()
